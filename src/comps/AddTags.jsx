...
const AddTags = () => {
    ...
    const addNewInputs = () => {
        ...
        const currentGroupCount = tagsContainer.querySelectorAll('.myTagsGroup').length + 1;
    ...
    const deleteLastInputs = () => {
        ...
        if (lastGroup) {
          lastGroup.remove();
          const remainingGroups = tagsContainer.querySelectorAll('.myTagsGroup').length;
  ...
  return (
    <div className="mb-2">
        <div>
            <p className="mb-2">Time to group your tags! <span className="font-bold">List Name</span> is the name of the button you will click to have the tags in <span className="font-bold">Tags List</span> be copied to your clipboard!</p>
        ...
        <div className="tagsContainer">
            <div className="my-3 flex gap-4 w-full myTagsGroup">
                <div className="flex flex-col myListTextBox">
                    <p className="text-[#dd7b94] font-bold mb-1">List Name:</p>
                    <input type="text" className="outline-0 myBody border border-black p-1" placeholder="Default Tags"/>
                </div>
                <div className="flex flex-col w-3/4 myTagsListTextBox">
                    <p className="text-[#dd7b94] font-bold mb-1">Tags List:</p>
                    <input type="text" className="outline-0 p-1 myBody border border-black" placeholder="study, studyblr, studyspo, study notes, study motivation, bullet journal, study blog"/>
                </div>
            </div>
        </div>
        {/* Buttons */}
        <div className="flex gap-4">
            <div className="border border-[#c5c5c5] w-fit bg-[#eeeeee] hover:bg-[#f9e1e7]" onClick={addNewInputs}>
                <button type="button" className="border border-t-white border-l-white p-1 font-medium" title="♡ Add new group"><HiPlus /></button>
            </div>
            {showMinusBtn && (
                <div className="border border-[#c5c5c5] w-fit bg-[#eeeeee] hover:bg-[#f9e1e7]">
                    <button type="button" className="border border-t-white border-l-white p-1 font-medium" title="♡ Delete last group" onClick={deleteLastInputs}><HiMinus /></button>
                </div>
            )}
        </div>
        {/* Test button */}
